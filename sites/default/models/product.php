...
class Product_Model extends Bl_Model
{
	...
	public function getTypeList()
	{
		...
		if (!isset($list)) {
			...
			if ($cache = cache::get($cacheId)) {
				...
				$result = $db->query('SELECT pt.*, count(distinct(p.sn)) productsCount FROM products_type pt
	...
	public function getTypeFieldWidget($field, $defaultValue = null, $defaultPrice = 0)
	{
		...
		switch ($field->display_type) {
			...
			case self::DISPLAY_TYPE_CHECKBOX:
				if ($multiple) {
					if ($fieldName == "field_fabric") {
						$html .= '<p>Main Fabric:<select name="main_fabric" class="non-empty" title="Main Fabric 不能为空">' . PHP_EOL;
						$html .= '<option value="">Select Main Fabric..</option>' . PHP_EOL;
						if (is_array($options) && !empty($options)) {
							foreach ($options as $option) {
								$html .= '<option value="' . plain($option) . '"' . ($option == $defaultValue[0] ? ' selected="selected"' : '') . '>' . plain($option) . '</option>' . PHP_EOL;
							}
						} else {
							$html .= '<option value=""></option>' . PHP_EOL;
						}
	...
	public function insertTypeField($type, $post)
	{
	  ...
	  'is_spec' => isset($post['is_spec']) ? $post['is_spec'] : 0,
		);
	...
	public function getProductsCount($post = array())
	{
		global $db;
		$filter = array(
      'products.pid' => isset($post['pid']) && trim($post['pid']) !== '' ? trim($post['pid']) : null,
      ...
      'free_shipping' => isset($post['free_shipping']) && $post['free_shipping'] !== '' ? intval($post['free_shipping']) : null,
		...
		if (isset($post['tids']) && $post['tids']) {
			...
			$termInfo->childsTid[] = $termInfo->tid;
			$db->join('products_directory', 'products_directory.pid = products.pid');
			$db->where('products_directory.tid IN', $termInfo->childsTid);
				
			$db->join('products_weight', 'products_weight.pid = products.pid');
			$db->where('products_weight.tid', $termInfo->tid);*/
			$db->join('products_weight', 'products_weight.pid = products.pid');
			$db->where('products_weight.tid', $post['tids']);
		}
		$db->select('count(distinct(sn))');
	...
	public function getProductsList($post = array(), $page = null, $pageRows = 60)
	{
      ...
      'free_shipping' => isset($post['free_shipping']) && $post['free_shipping'] !== '' ? intval($post['free_shipping']) : null,
		...
		$db->select('distinct products.pid');
		foreach ($filter as $key => $value) {
			if (isset($value) && $value !== '' && $value !== false) {
				$db->where($key, $value);
			}
		...
		if (isset($post['orderby']) && $post['orderby']) {
			...
			$db->orderby('status DESC, pid DESC');
		}
		if (isset($page) && $pageRows != 'all') {
			$db->limitPage($pageRows, $page);
		}
		//added to remove products with same sn.
		//$db->groupby('sn');
		$result = $db->get('products');
		$pids = $result->column();
		$products = array();
	...
	public function getProductsListBySpecial($post = array(), $page = 1, $pageRows = 60)
	{
      ...
      'p.brand_tid' => isset($post['brand_tid']) && $post['brand_tid'] !== '' ? intval($post['brand_tid']) : null,
		...
		*/
		if (isset($post['special_tid']) && $post['special_tid']) {
			$db->where('tp.tid', $post['special_tid']);
		}
		$db->join('products p', 'tp.pid = p.pid');
		if (isset($post['orderby']) && $post['orderby']) {
			$db->orderby($post['orderby']);
	...
	public function getTermByPath($path){
		global $db;
		$result=$db->query('select * from terms where path_alias="'.$path.'"');
		return $result->one();
	}
	...
	public function getProductRealPrice(&$product)
	{
		...
		if (!empty($product->tags) && key_exists('onsale', $product->tags)) {
			$product->price *= Bl_Config::get('discount', 1);
		}
		/* 老的代码可以删除
		global $user;
		$pid = $product->pid;
	...
	public function getProduct_directoryBypid($pid){
		global $db;
		$result=$db->query('select * from products_directory join terms on products_directory.tid=terms.tid where pid="'.$pid.'"');
		return $result->all();
	}
	...
	public function updataProcduct_Directory($pid,$tid,$pdid){
		global $db;
		$sql="update products_directory SET pid=".$pid." tid=".$tid." where pdid=".$pdid;
		$set = array(
				'pid'=>$pid,
				'tid'=>$tid,
		);
		$db->update('products_directory', $set, array('pdid' => $pdid));
		return $db->affected()>0;
	}
	
	
	public function insertProduct_Directory($pid,$tid){
		global $db;
	    $taxonomyInstance = Taxonomy_Model::getInstance();
	    $termInfo = $taxonomyInstance->getTermInfo($tid);
	    if(isset($termInfo)){
    		$list=$this->getProduct_directoryBypid($pid);
    		$insert=true;
    		if (isset($list)) {
        		foreach($list as $pd){
        			if($pd->tid==$tid||$pd->ptid1==$tid||$pd->ptid2==$tid||$pd->ptid3==$tid){
        				$insert=false;
        				break;
        			}
        			else if($termInfo->ptid1==$pd->tid||$termInfo->ptid2==$pd->tid||$termInfo->ptid3==$pd->tid){
        				$this->updataProcduct_Directory($pid, $tid, $pd->pdid);
        				$insert=false;
        				break;
        			}    
        		}
    		...
    		if($insert){
    		    ...
    		    $sql="insert into products_directory values(".$pid.",".$tid.")";
    		$set=array(
    			'pid'=>$pid,
    		    'tid'=>$tid,
    		);
    		$db->insert("products_directory", $set);
	...
	public function deleteProduct_Directory($pdid){
		global $db;
		$db->delete("products_directory",array('pdid'=>$pdid));
	}
	
	public function deleteProduct_DirectoryByPidTid($pid, $tid) {
		global $db;
		$db->delete("products_directory",array('pid'=>$pid, 'tid' => $tid));
	}
	 ...
	 * @param productInfo
	 */
	public function copyProduct($productInfo) {
		$latestNumber = $this->getLatestProductNumberBySn($productInfo->sn);
		
		$copyVersion = 0;
		if (preg_match('/^[0-9a-zA-z]+v(\d+)$/', $latestNumber, $matches)) {
			$copyVersion = intval($matches[1]);
		}
		$productInfo->number = $productInfo->number . 'v' . (++$copyVersion);
		$set = array(
				...
				'shippable' => $productInfo->shippable,
				'free_shipping' => $productInfo->free_shipping,
				'directory_tid1' => $productInfo->directory_tid1,
				'directory_tid2' => $productInfo->directory_tid2,
				'directory_tid3' => $productInfo->directory_tid3,
				'directory_tid4' => $productInfo->directory_tid4,
				'brand_tid' => $productInfo->brand_tid,
				'sell_price' => $productInfo->sell_price,
				'list_price' => $productInfo->list_price,
				'wt' => $productInfo->wt,
				'stock' => $productInfo->stock,
				'sell_min' => $productInfo->sell_min,
				'sell_max' => $productInfo->sell_max,
				'fid' => $productInfo->fid,
				'filepath' => $productInfo->filepath,
				'videopath' => $productInfo->videopath,
				'summary' => $productInfo->summary,
				'description' => $productInfo->description,
				'path_alias' => $productInfo->path_alias,
				'template' => $productInfo->template,
				'visits' => $productInfo->visits,
				'transactions' => $productInfo->transactions,
				'created' => $productInfo->created,
				'updated' => $productInfo->updated,
				'pvid' => $productInfo->pvid,
				'visible' => $productInfo->visible,
				'weight' => $productInfo->weight,
				'customizable' => $productInfo->customizable,
				'sphinx_key' => $productInfo->sphinx_key,
		);
		global $db;
		$db->insert('products', $set);
		return $db->affected() > 0;
		//return $this->insertProduct($set);
	}
	 ...
	 * @param string $sn
	 */
	public function getLatestProductNumberBySn($sn) {
		global $db;
		$db->select('number');
		$db->where('sn', $sn);
		$db->orderby('pid desc');
		$result = $db->get('products', 1);
	...
	public function insertProductFields($pid, $type, $post)
	{
		...
		if ($fieldsList) {
			...
			$fileInstance = File_Model::getInstance();
			foreach ($fieldsList as $fieldName => $field) {
				if (isset($post->{'field_' . $fieldName}) || isset($_FILES['field_' . $fieldName])) {
					...
					if ($field->multiple) {
						if ($field->display_type == self::DISPLAY_TYPE_FILE) {
							...
							$data = is_array($post->{'field_' . $fieldName}) ? $post->{'field_' . $fieldName} : array($post->{'field_' . $fieldName});
						}
						if ($fieldName == "fabric") {
							$main_fabric = $post->main_fabric;
							$temp = array();
							if (in_array($main_fabric, $data)) {
								$index = 1;
								foreach ($data as $delta => $value) {
									if ($value == $main_fabric) {
										$temp[0] = $main_fabric;
									} else {
										$temp[$index++] = $value;
									}
	...
	public function getProductIdsBySn($sn){
		global $db;
		$db->select('pid');
		$db->where('sn', $sn);
		$result = $db->get('products');
	 ...
	 * @return boolean
	 */
	public function updateProductDirectory($pid,$post)
	{
		global $db;
		if (isset($post['name']) && '' == $post['name']) {
			return false;
		...
		if ($fieldsList) {
			foreach ($fieldsList as $fieldName => $field) {
				if ($field->multiple) {
					...
					$defaultValue = "";
					if($filed->filed_type == self::FIELD_TYPE_INTEGER || $filed->filed_type == self::FIELD_TYPE_FILE) {
						$defaultValue = 0;
					}
					$db->update($this->getTypeTableName($type), array($this->getTypeFieldName($fieldName) => $defaultValue), array('pid' => $pid));
				}
			}
		}
	}
	...
	public function deleteProduct($pid)
	{
		...
		if ($productInfo = $this->getProductInfo($pid)) {
			...
			if ($affected) {
				...
				$db->exec('DELETE FROM products_directory WHERE pid = ' . $sqlPid);
				$db->exec('DELETE FROM products_weight WHERE pid = ' . $sqlPid);
				cache::remove('product-type');
				cache::remove('product-' . $pid);
			}
		}
	 ...
	 * @return boolean
	 */
	public function deleteProductCacheImage($pid)
	{
		$ret = true;
		$filelist = $this->getProductFilesList($pid);
		foreach ($filelist as $file) {
			$filelist = glob(DOCROOT . '/images/cache/*/' . $file->filepath);
			foreach($filelist as $file){
				if(!unlink($file)) {
					$ret = false;
					break;
	...
	public function listProductRelated($pid, $page = 0, $pageRows = 60)
	{
		...
		if (!isset($list[$pid])) {
			...
			if ($cache = cache::get($cacheId)) {
				...
				$db->from('products_relations');
				$db->join(array('b'=>'(select distinct sn, min(pid) as pid from products group by sn)'), 'b.pid = related_pid');
				$db->where('products_relations.pid', $pid);
	...
	function getProductTermsInfo($pid)
	{
		global $db;
		//static $list = array();
		$pid=intval($pid);
		...
		$sql='select * from products_directory join terms on products_directory.tid=terms.tid Where pid="'.$db->escape($pid).'"';
		$result=$db->query($sql);
		return $result->all();
	}
	...
	{
		global $db;
		$db->insert('terms_products', array('pid' => $pid, 'tid' => $tid), true);
	}
	...
	public function searchProductSnByTerm($tid) {
		global $db;
		$sql = sprintf("select sn from products, terms_products where products.pid = terms_products.pid and terms_products.tid = %d", $tid);
		$result = $db->exec($sql);
		if ($result === false) {
			return null;
		}
		return $result->allWithKey('sn');
	}
	/**
	 * 获取商品最大价格，最小价格
	...
	public function getReviewCountUnderTerm($termInfo) {
	    global $db;
	    $tid = $termInfo->tid;
	    $termInfo->childsTid[] = $tid;
	    if ($termInfo->vid == Taxonomy_Model::TYPE_DIRECTORY) {
		    $sql = sprintf('select count(distinct cid) from products_comments,products,products_directory where products_comments.pid = products.pid  
		    		and products_directory.pid = products.pid and products_directory.tid in 
		    		(%s)', implode(',', $termInfo->childsTid));
		    $result = $db->query($sql);
		    $count = (int)$result->one();
	    } else {
	    	$sql = sprintf('select count(distinct cid) from products_comments,products,terms_products where products_comments.pid = products.pid
	    		and terms_products.pid = products.pid and terms_products.tid in
	    		(%s)', implode(',', $termInfo->childsTid));
	    	$result = $db->query($sql);
	    	$count = (int)$result->one();
	    }
	    if ($count == 0) {
	    	$count = 100 + base_convert(md5($termInfo->name), 16, 10) % 300;
	    }
	    return $count;
	}
	public function getPopularCommentedProducts($termInfo, $limit = -1){
		global $db;
		$tid = $termInfo->tid;
		$termInfo->childsTid[] = $tid;
		if ($termInfo->vid == Taxonomy_Model::TYPE_DIRECTORY) {
		$sql = sprintf('select products_directory.tid, products.pid, products.sn, count(comments.cid) as comments_count, products.name, products.list_price, products.sell_price, products.filepath,products.path_alias
				from products_directory, products,products_comments, comments where products_directory.pid = products.pid and products_directory.tid in
		    	(%s) and products.pid = products_comments.pid
				and products_comments.cid = comments.cid and comments.status = 2 and products.status = 1 and (comments.directory_tid = %d or comments.directory_tid = 0)
				group by products.pid order by max(comments.timestamp) desc, products.pid desc ', implode(',', $termInfo->childsTid), $tid);
		} else {
			$sql = sprintf('select terms_products.tid, products.pid, products.sn, count(comments.cid) as comments_count, products.name, products.list_price, products.sell_price, products.filepath,products.path_alias
				from terms_products, products,products_comments, comments where terms_products.pid = products.pid and terms_products.tid in
		    	(%s) and products.pid = products_comments.pid
				and products_comments.cid = comments.cid and comments.status = 2 and products.status = 1 and (comments.tag_tid = %d or comments.tag_tid = 0)
				group by products.pid order by max(comments.tag_tid) desc, max(comments.timestamp) desc, products.pid desc ', implode(',', $termInfo->childsTid), $tid);
		}
		if ($limit != -1) {
			$sql .= ' limit ' . $limit;
		}
	 ...
	 * 获取原创的reivew
	 * @todo 需要重构
	 * @param unknown $termInfo
	 * @param unknown $limit
	 */
	public function getHumanCommentedProducts($termInfo, $limit = -1) {
		global $db;
		$tid = $termInfo->tid;
		$productsList = null;
		$cacheId = 'humanCommentedProducts-' . $tid;
		if ($cache = cache::get($cacheId)) {
			$productsList = $cache->data;
		} else {
			$termInfo->childsTid[] = $tid;
			if ($termInfo->vid == Taxonomy_Model::TYPE_DIRECTORY) {
				$sql = sprintf('select products_directory.tid, products.pid, products.sn, count(comments.cid) as comments_count, products.name, products.list_price, products.sell_price, products.filepath,products.path_alias
				...
				$sql = sprintf('select terms_products.tid, products.pid, products.sn, count(comments.cid) as comments_count, products.name, products.list_price, products.sell_price, products.filepath,products.path_alias
					from terms_products, products,products_comments, comments where terms_products.pid = products.pid and terms_products.tid in
			    	(%s) and products.pid = products_comments.pid
					and products_comments.cid = comments.cid and comments.status = 2 and products.status = 1
					group by products.pid order by max(comments.timestamp) desc, products.pid desc ', implode(',', $termInfo->childsTid));
			}
			if ($limit != -1) {
				$sql .= ' limit ' . $limit;
			}
			$result = $db->exec($sql);
			$productsList = $result->all();
			cache::save($cacheId, $productsList);
		}
		return $productsList;;
	}
	...
	public function getPopularCommentedProductsByPage($tid, $page = 1, $pageRows = 15){
		...
		$sql = sprintf('select products_directory.tid, products.pid, products.sn, count(comments.cid) as comments_count, products.name, products.list_price, products.sell_price, products.filepath,products.path_alias
				from products_directory, products,products_comments, comments where products_directory.pid = products.pid and products_directory.tid in
		    	(%s) and products.pid = products_comments.pid
				and products_comments.cid = comments.cid and comments.status >= 1 and products.status = 1
				group by products.pid order by products.pid desc limit %d, %d', implode(',', $termInfo->childsTid), $pageRows *　($page - 1), $pageRows);
		} else {
			$sql = sprintf('select terms_products.tid, products.pid, products.sn, count(comments.cid) as comments_count, products.name, products.list_price, products.sell_price, products.filepath,products.path_alias
				from terms_products, products,products_comments, comments where terms_products.pid = products.pid and terms_products.tid in
		    	(%s) and products.pid = products_comments.pid
	...
	public function getCommentdProductsByPage($page = 1, $pageRows = 15) {
		global $db;
		$sql = sprintf('select products.pid, products.sn, count(comments.cid) as comments_count, products.name, products.list_price, products.sell_price, products.filepath,products.path_alias
				from products, products_comments, comments where products.pid = products_comments.pid
				and products_comments.cid = comments.cid and comments.status >= 1 and products.status = 1
				group by products.pid order by max(comments.status) desc, max(comments.timestamp) desc, products.pid desc limit %d, %d', $pageRows * ($page - 1), $pageRows);
		$result = $db->exec($sql);
		$commentedProducts = $result->all();
		return $commentedProducts;
	}
	
	public function getCommentdProductsCount() {
		global $db;
		$sql = 'select count(distinct products.pid)
				from products, products_comments, comments where products.pid = products_comments.pid
				and products_comments.cid = comments.cid and comments.status >= 1 and products.status = 1';
		$result = $db->exec($sql);
		$count = $result->one();
	...
	public function getAttrValuesList($productType, $attr)
	{
	    global $db;
	    $sql = sprintf('select settings from products_type_fields where field_name = "%s" and type="%s"', $attr, $productType);
	    $result = $db->query($sql);
	    $settings = unserialize($result->one());
	    //$tb = $this->getTypeFieldTableName($productType, $attr);
	    return explode("\r\n", $settings["options"]);
	}
}
