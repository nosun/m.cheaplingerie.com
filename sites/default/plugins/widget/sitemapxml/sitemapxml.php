...
class Sitemapxml extends Widget_Abstract
{
  ...
  public function getXML($gzip = false){
  	...
  	if ($cache = cache::get($cacheId)) {
        ...
        $xml .= '    <sitemap>' . PHP_EOL;
        $xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_home.xml.gz</loc>' . PHP_EOL;
        $xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
        $xml .= '    </sitemap>' . PHP_EOL;
        
        $xml .= '    <sitemap>' . PHP_EOL;
        $xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_term.xml.gz</loc>' . PHP_EOL;
        $xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
        $xml .= '    </sitemap>' . PHP_EOL;
        
        $xml .= '    <sitemap>' . PHP_EOL;
        $xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_tag.xml.gz</loc>' . PHP_EOL;
        $xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
        $xml .= '    </sitemap>' . PHP_EOL;
        
        $xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_1.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_2.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_3.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_4.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_5.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_6.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_7.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_8.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_autotag_9.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
		
		$xml .= '    <sitemap>' . PHP_EOL;
		$xml .= '       <loc>' . $domainUrl . '/sitemap/sitemap_products.xml.gz</loc>' . PHP_EOL;
		$xml .= '		<lastmod>' . date('Y-m-d', time()) . '</lastmod>' . PHP_EOL;
		$xml .= '    </sitemap>' . PHP_EOL;
...
//             case 'term':
//               $result = $db->query('SELECT t.path_alias tpa FROM terms t WHERE (t.vid = 3 or t.vid = 1) AND visible = 1');
//               $terms = $result->column();
//               foreach ($terms as $tpa) {
//                 $tpa = str_replace('&', '', $tpa);
//                 $xml .= $this->getUrlXML($tpa . '.html', TIMESTAMP, $setting);
//               }
//               break;
...
//             case 'product':
//               $result = $db->query('SELECT sn, path_alias pa, updated FROM products WHERE status = 1');
//               $products = $result->all();
//               foreach ($products as $pa) {
//                 $purl = str_replace('&', '', trim($pa->pa) .'-p'.$pa->sn. '.html');
//                 $xml .= $this->getUrlXML($purl, TIMESTAMP, $setting);
//               }
//               break;
